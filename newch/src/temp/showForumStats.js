import { getDupeSortedDictionary } from '../utils';
function execScript() {
	...
	const helper = (el) => Array.from(document.querySelectorAll(el)).map(e => e.innerText);

	const publishersDict = getDupeSortedDictionary(helper("#threads .threadinfo .username"));	
	const commentorsDict = getDupeSortedDictionary(helper("#threads .threadlastpost .username"));
	const words = helper("#threads .title").map(t => t.match(/([^\s.,\/#?!$%\^&\*+;:{}|=\-_`~()]+)/g).filter(w => w.length > 1))
	const wordsDict = getDupeSortedDictionary(words);
	const prefixesDict = getDupeSortedDictionary(helper("#threads .prefix").map(p => p.replace(/\||סקר: /, '')));
